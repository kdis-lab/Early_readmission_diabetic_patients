...
mydata <- read.csv("diabetic_data-fourthStep-preproc.csv", sep = ",", na.strings = "")
...
for(i in 1: (ncolumns-1)){
  ...
  for(j in (i+1):ncolumns) {
    ...
    if((class(mydata[,firstCol])=="integer" || class(mydata[,firstCol])=="numeric") 
      ...
      #Correlation  is  an  effect  size  and  so  we  can  verbally  describe  the  strength  
      #of  the correlation using the guide that Evans (1996) suggests for the absolute value of 
      #r:
      #.00 - .19 very weak
      #.20 - .39 weak
      #.40 - .59 moderate
    ...
    if(class(mydata[,firstCol])=="factor" && class(mydata[,secondCol])=="factor"){
      ...
      #which is simply a Pearson correlation computed on dichotomous variables. 
      #In this case, phi coefficient and Crammer match.
      ...
      #In practice, you may find that a Cramer's V of .10 provides a good minimum
      #threshold for suggesting there is a substantive relationship between two variables.
      
      test <- assocstats(table(mydata[,firstCol],mydata[,secondCol]))
      
      test <- summary(test)
      
      levelsFirst <- levels(mydata[,firstCol])
      levelsSecond <- levels(mydata[,secondCol])
      
      if(levelsFirst==levelsSecond){
        
        #When the data matrix is 2x2, the Phi statistic is used.
        if(levelsFirst==2){
          corre <- test$object$phi
          
        } else{
          #When the number of rows and columns in the data matrix is the same, but greater than 2, 
          #the Contingency Coefficient is employed.
          corre <- test$object$contingency
        }
      } else{
        #Cramer's V is used when the number of rows and columns is unequal.
        corre <- test$object$cramer        
      }
      
      #A general rule of thumb for interpreting the strength of associations between nominal vars is:
    ...
    if((class(mydata[,firstCol])=="factor" && class(mydata[,secondCol])=="integer") 
       ...
       || (class(mydata[,firstCol])=="factor" && class(mydata[,secondCol])=="numeric"))
      {
        ...
        #The strength of correlation between a categorical variable (dichotomous) 
        #and an continuous variable can be computed using point biserial correlation.
        #It is mathematically equivalent to the Pearson (product moment) correlation, that is, 
        #if we have one continuously measured variable X and a dichotomous variable Y.
        if(length(levelsVar)==2){
          ...
          #The rule of stregth's interpretation could be the same as in the r coefficient.
          ...
          p.value <- unlist(summary(anovaTest))[["Pr(>F)1"]]
